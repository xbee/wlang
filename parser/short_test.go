...
var valids = []string{
	...
	`package p; fn f() { _ = <-chan int(nil) };`,
	`package p; fn f() { _ = (<-chan int)(nil) };`,
	`package p; fn f() { _ = (<-chan <-chan int)(nil) };`,
	`package p; fn f() { _ = <-chan <-chan <-chan <-chan <-int(nil) };`,
	...
	`package p; fn f() { select { case <- c: case c <- d: case c <- <- d: case <-c <- d: } };`,
	`package p; fn f() { select { case x := (<-c): } };`,
	...
	`package p; fn _(x chan int) { chan int(x) <- 0 }`,
...
var invalids = []string{
	...
	`package p; fn f() { select { case _ <- chan /* ERROR "expected expression" */ int: } };`,
	`package p; fn f() { _ = (<-<- /* ERROR "expected 'chan'" */ chan int)(nil) };`,
	`package p; fn f() { _ = (<-chan<-chan<-chan<-chan<-chan<- /* ERROR "expected channel type" */ int)(nil) };`,
	...
	`package p; fn f() { for i /* ERROR "boolean or range expression" */ , x := []string {} }`,
	`package p; fn f() { go f /* ERROR HERE "function must be invoked" */ }`,
	`package p; fn f() { defer fn() {} /* ERROR HERE "function must be invoked" */ }`,
	`package p; fn f() { go fn() { fn() { f(x fn /* ERROR "missing ','" */ (){}) } } }`,
